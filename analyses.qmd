...
Using the tibble you've developed following the instructions on the [Data](data.qmd) and [Proposal](proposal.qmd) pages, you will perform three analyses, as specified below. Your final portfolio report will include:
...
5. assess whether fitting a robust linear model (using the `rlm()` function in the `MASS` package) produces a meaningfully different fit than an ordinary least squares model. (If so, this would indicate that your OLS model may be influenced by outliers.)
